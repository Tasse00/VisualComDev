...
const ComWrapper: React.FC<ComWrapperProps> = props => {
    ...
    const [{ isOver, isOverCurrent }, drop] = useDrop<{type:string; data:Widget;}, void, {isOver: boolean, isOverCurrent: boolean}>({
        ...
        drop(item, monitor) {
            ...
            dispatch({ type: ActTypes.ADD_WIDGET, payload: { containerId: props.nodeId, widgetType: item.data.type } });
        },
        collect: (monitor) => ({
    ...
    return (
        <div ref={drop} className={styles['com-wrapper']} style={props.style}>
            {props.children}
            <div className={styles['com-wrapper-mask']} style={maskStyle} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} onClick={props.onClick}></div>
        </div>
    );
}
