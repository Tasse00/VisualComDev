...
export type AvailableAction =
  | ActRegisterInstanceFeatures
  | ActRemoveInstanceFeatures
  | ActSyncListenerConfigs
  | ActEmitEvent;
...
export function reducer(state: State, action: AvailableAction): State {
  switch (action.type) {
    case 'register-instance-features': {
      ...
      const instanceFeatures = (state.featuresMap[instanceId] = {} as {
        [name: string]: VCD.InstanceFeature;
      });
      features.map((feat) => {
    ...
    case 'remove-instance-features': {
      ...
      logger.debug('removed instance', instanceId, 'features');
      return { ...state, featuresMap: { ...state.featuresMap } };
    }
    case 'sync-listener-configs': {
      const { sourceId, listeners } = action.payload;
      state.listeners[sourceId] = listeners;
      logger.debug('sync instance', sourceId, listeners.length, 'listeners');
    ...
    case 'emit-event': {
      ...
      for (let [sourceId, listeners] of Object.entries(state.listeners)) {
        ...
        listeners
          .filter(
            (listener) =>
              (listener.target === triggerId || !listener.target) &&
          ...
          .map((matchedListener) => {
            ...
            try {
              featureCallback =
                state.featuresMap[sourceId][matchedListener.feature].callback;
            } catch (e) {
              logger.warning(
                'instanceId',
                sourceId,
                'has no feature named',
                matchedListener.feature,
              );
              return state;
            }
