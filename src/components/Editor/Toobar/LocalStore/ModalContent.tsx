...
import { useEditor, useEditorInstances, useStoredPages } from '../../Providers/Editor/hooks';
      ...
      <List
        ...
        renderItem={(store) => (
          <List.Item
            actions={[
              ...(tree ? (
                [<Button icon={<UploadOutlined />} onClick={() => update(store.guid, {
                  guid: store.guid,
                  name: tree.name,
                  tree,
                  timestamp: new Date().getTime(),
              ...
              tree ? (
                ...
                >
                  <Button key="load" icon={<DownloadOutlined />} >加载</Button>
                </Popconfirm>
              ) : (
                  <Button key="load" icon={<DownloadOutlined />} onClick={() => {
                    if (store.tree) {
                      dispatch({
                        type: 'load-tree',
                        payload: { tree: store.tree },
